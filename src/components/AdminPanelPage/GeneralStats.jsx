...
export default function GeneralStats() {
    ...
    return (
        <div className="flex gap-8 ">
            {/* Card 1 */}
            <div className="bg-neutral-50/60 border border-neutral-100 flex flex-col items-center justify-center px-6 py-8 shadow-md rounded-lg ">
                <h2>Total de productos</h2>
                <p className="font-bold text-2xl">{loading ? "..." : stats.products}</p>
            </div>

            {/* Card 2 */}
            <div className="bg-neutral-50/60 border border-neutral-100 flex flex-col items-center justify-center px-6 py-8 shadow-md rounded-lg ">
                <h2>Total de Marcas</h2>
                <p className="font-bold text-2xl">{loading ? "..." : stats.brands}</p>
            </div>

            {/* Card 3 */}
            <div className="bg-neutral-50/60 border border-neutral-100 flex flex-col items-center justify-center px-6 py-8 shadow-md rounded-lg ">
                <h2>Total de Categorias</h2>
                <p className="font-bold text-2xl">{loading ? "..." : stats.categories}</p>
            </div>

            {/* Card 4 */}
            <div className="bg-neutral-50/60 border border-neutral-100 flex flex-col items-center justify-center px-6 py-8 shadow-md rounded-lg ">
                <h2>Productos Visibles</h2>
                <p className="font-bold text-2xl">{loading ? "..." : stats.visibleProducts}</p>
            </div>
        </div>
    );
}
